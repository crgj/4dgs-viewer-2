...
#### 选择工具增强
- **时间感知选择**: 选择工具现在只选择当前时刻可见的点（基于生命周期）
- **撤销/重做功能**: 删除操作支持最多 30 步撤销/重做
  - `Delete` 键删除选中点
...
| `2` | 切换矩形工具 | 按开启，再按关闭 |
| `Alt` + 拖拽 | 减选模式 | 从选区中移除点 |
| `Delete` | 删除选中 | 删除当前选中的点 |
| `Ctrl+Z` | 撤销 | 撤销删除操作（最多30步） |
| `Ctrl+Y` | 重做 | 重做删除操作 |
| `Ctrl+Shift+Z` | 重做 | 重做的另一种快捷键 |
| `ESC` | 关闭弹窗 | 关闭帮助/设置弹窗 |

#### 选择模式说明
- **中心模式**: 只选择中心点位于选择区内的点，适合精确选择
- **椭圆模式**: 考虑点的屏幕空间大小，适合选择较大的高斯点

#### 时间感知选择
选择工具现在是时间感知的！只会在当前时刻可见的点中进行选择：
- 播放动画时，当前时刻会自动更新
- 静止时，只选择该静止时刻可见的点
- 通过点的生命周期 (`lifetime_mu`, `lifetime_w`) 判断可见性

> 💡 **提示**: 点击工具栏的 `?` 按钮可随时查看快捷键帮助
