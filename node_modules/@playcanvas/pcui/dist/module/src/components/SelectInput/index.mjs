...
class SelectInput extends Element {
    ...
    constructor(args = {}) {
        ...
        this._onInputKeyDown = (evt) => {
            if (evt.key === 'Enter' && this.enabled && !this.readOnly) {
                ...
                if (value !== undefined) {
                    // For tags-based inputs with allowCreate, validate that the value is not empty or whitespace-only
                    if (this._allowCreate && typeof value === 'string' && value.trim() === '') {
                        return;
                    }
                    this.focus();
                    this.close();
    ...
    open() {
        ...
        this._containerOptions.forEachChild((child) => {
            const label = child;
            label.hidden = false;
